...
struct DBImpl::CompactionState {
  ...
  explicit CompactionState(Compaction* c) : compaction(c), smallest_snapshot(0), outfile(nullptr), builder(nullptr), total_bytes(0) {}
    ...
    if (ParseFileName(filename, &number, &type)) {
      ...
      switch (type) {
        ...
        case kTableFile: {
          ...
          if (!keep) {
            kGlobalInfo[number].delete_time = std::chrono::high_resolution_clock::now();
            kGlobalInfo[number].live_time = std::chrono::duration_cast<std::chrono::milliseconds>(kGlobalInfo[number].delete_time - kGlobalInfo[number].create_time).count();
            kGlobalInfo[number].is_live = false;
          }
        }
...
Status DBImpl::RecoverLogFile(uint64_t log_number, bool last_log, bool* save_manifest, VersionEdit* edit, SequenceNumber* max_sequence) {
  struct LogReporter : public log::Reader::Reporter {
    ...
    void Corruption(size_t bytes, const Status& s) override {
      Log(info_log, "%s%s: dropping %d bytes; %s", (this->status == nullptr ? "(ignoring error) " : ""), fname, static_cast<int>(bytes), s.ToString().c_str());
...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit, Version* base) {
  ...
  Status s;
  {
    ...
    kGlobalInfo[meta.number].type = FileType::kTableFile;
    kGlobalInfo[meta.number].level = 0;
    kGlobalInfo[meta.number].create_time = std::chrono::high_resolution_clock::now();
    kGlobalInfo[meta.number].file_size = meta.file_size;
    kGlobalInfo[meta.number].smallest_key = meta.smallest.user_key().ToString();
    kGlobalInfo[meta.number].largest_key = meta.largest.user_key().ToString();
    kGlobalInfo[meta.number].is_live = true;
    mutex_.Lock();
  }
...
Status DBImpl::FinishCompactionOutputFile(CompactionState* compact, Iterator* input) {
  ...
  kGlobalInfo[compact->current_output()->number].is_live = true;
  kGlobalInfo[compact->current_output()->number].type = FileType::kTableFile;
  kGlobalInfo[compact->current_output()->number].level = compact->compaction->level() + 1;
  kGlobalInfo[compact->current_output()->number].create_time = std::chrono::high_resolution_clock::now();
  kGlobalInfo[compact->current_output()->number].file_size = current_bytes;
  kGlobalInfo[compact->current_output()->number].smallest_key = compact->current_output()->smallest.user_key().ToString();
  kGlobalInfo[compact->current_output()->number].largest_key = compact->current_output()->largest.user_key().ToString();
  delete compact->outfile;
  ...
  if (s.ok() && current_entries > 0) {
    ...
    delete iter;
    if (s.ok()) {
      Log(options_.info_log, "Generated table #%llu@%d: %lld keys, %lld bytes", (unsigned long long)output_number, compact->compaction->level(), (unsigned long long)current_entries,
...
struct IterState {
  ...
  IterState(port::Mutex* mutex, MemTable* mem, MemTable* imm, Version* version) : mu(mutex), version(version), mem(mem), imm(imm) {}
};
  ...
  if (in.starts_with("num-files-at-level")) {
    ...
    for (int level = 0; level < config::kNumLevels; level++) {
      ...
      if (stats_[level].micros > 0 || files > 0) {
        std::snprintf(buf, sizeof(buf), "%3d %8d %8.0f %9.0f %8.0f %9.0f\n", level, files, versions_->NumLevelBytes(level) / 1048576.0, stats_[level].micros / 1e6,
